...
function [W,XB,error,error2, cond_nums, last_x] = TLConditioned(W,Y,numiter, STY, kappa, targetWnorm)
...
for i=1:numiter
    ...
    if isempty(kappas)
        ...
        if kappa <= 2
            ...
            % add a perturbation to fix the convergence discontinuities
            cll=1e-1;
            while(abs(det(W)) <= 10^(-250))
                W = W + ((rand(K,n) - 0.5)*cll);
            end
